...
describe("Authentication", () => {
  test("User is able to Sign Up only once", async () => {
    ...
    const userExists = await axios.post(`${BACKEND_URL}/api/v1/user/signup`, {
      username,
      password,
      type: "admin",
    });

    expect(userExists.statusCode).toBe(400);
  });

  test("Signup fails if the usernmae is empty", async () => {
    const username = "user" + Math.random();
  ...
  test("Signup fails if the username and password is correct", async () => {
    const username = "user" + Math.random();
    const password = "Password@1234";
    const response = axios.post(`${BACKEND_URL}/api/v1/signup`, {
      username,
  ...
  test("Signup fails if the username and password is incorrect", async () => {
    const username = "user" + Math.random();
    const password = "Password@1234";
    const response = axios.post(`${BACKEND_URL}/api/v1/signup`, {
      username,
